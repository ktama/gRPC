...
var request = new DeviceSysLogRequest() { Level = 1 };
var deviceLog = client.GetDeviceLog(request);
Console.WriteLine($"Ticks: {new DateTime(deviceLog.Ticks):yy/MM/dd(ddd) HH:mm:ss.fffff} / LogLevel: {deviceLog.Level} / LogMessage: {deviceLog.Message}");
request = new DeviceSysLogRequest() { Level = 2 };
deviceLog = client.GetDeviceLog(request);
Console.WriteLine($"Ticks: {new DateTime(deviceLog.Ticks):yy/MM/dd(ddd) HH:mm:ss.fffff} / LogLevel: {deviceLog.Level} / LogMessage: {deviceLog.Message}");
request = new DeviceSysLogRequest() { Level = 3 };
deviceLog = client.GetDeviceLog(request);
Console.WriteLine($"Ticks: {new DateTime(deviceLog.Ticks):yy/MM/dd(ddd) HH:mm:ss.fffff} / LogLevel: {deviceLog.Level} / LogMessage: {deviceLog.Message}");
